<template>
  <div id="user_base">
    ...
    <div id="mainbox">
      <router-link :to="'/user/bookDetail?'+$route.query.id" id="back">
        <i class="el-icon-arrow-left"></i>
        <span>返回</span>
      </router-link>
      <el-form ref="form" :model="form" label-width="80px">
        <span style="margin-bottom:50px;display:block;">读者信息</span>
...
export default {
  ...
  mounted() {
    this.getReaderInfo(this.$route.query.sno);
  },
  methods: {
    getReaderInfo(account) {
      var that = this;
      this.$http
        .post("/api/user/ReaderMsg", {
          sno: account
        })
        .then(response => {
